...
```bash
brew tap user/tap
brew install --cask tiktok-business-gateway
```
...
swift run tiktok-business-gateway-reader --version | tail -n 1 | grep -Fx "$version"
swift run tiktok-business-gateway-writer --version | tail -n 1 | grep -Fx "$version"
```
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/tiktok-business-gateway-${version}-darwin-arm64.dmg"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/tiktok-business-gateway-${version}-darwin-x64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/tiktok-business-gateway-${version}-darwin-arm64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/tiktok-business-gateway-${version}-darwin-x64.dmg"
```
...
`../homebrew-tap/Casks/tiktok-business-gateway.rb`.

After reviewing the rendered tap Cask:

```bash
cd ../homebrew-tap
git add Casks/tiktok-business-gateway.rb README.md
git diff --staged --stat
git commit -m "chore: release tiktok-business-gateway <version>"
...
verify `api/cask/tiktok-business-gateway.json` from GitHub
