...
writer_product="tiktok-business-gateway-writer"
...
main() {
  ...
  release_base_url="${CASK_RELEASE_BASE_URL:-https://github.com/tacogips/tiktok-business-gateway/releases/download/v$version}"
  ...
  cat > "$output" <<EOF
cask "tiktok-business-gateway" do
  version "$version"
  ...
  url "$release_base_url/$artifact_name-#{version}-#{arch}.dmg",
      verified: "github.com/tacogips/tiktok-business-gateway/releases/download/"
  name "tiktok-business-gateway"
  desc "Bounded TikTok Marketing API reader and status-update gateway"
  homepage "https://github.com/tacogips/tiktok-business-gateway"
  ...
  caveats do
    <<~EOS
      ...
      Homebrew links $reader_product and $writer_product into the native Homebrew prefix for this Mac.
    EOS
  end
end
EOF
