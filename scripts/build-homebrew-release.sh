...
publish_new_file() {
  ...
  if [[ -e "$destination" || -L "$destination" ]]; then
    printf 'refusing to replace existing release output: %s\n' "$destination" >&2
    return 1
  fi
  mv -n -- "$source" "$destination"
  if [[ -e "$source" || ! -f "$destination" || -L "$destination" ]]; then
    printf 'release output appeared concurrently: %s\n' "$destination" >&2
    return 1
  fi
}
...
build_target() {
  local version target release_dir bin_path work_root build_root work_dir archive archive_base
  local temporary_archive temporary_checksum product
  version="$1"
  target="$2"
  release_dir="$3"
  archive="$release_dir/$artifact_name-$version-$target.tar.gz"
  ...
  if [[ -e "$archive" || -L "$archive" || -e "$archive.sha256" || -L "$archive.sha256" ]]; then
    printf 'refusing to replace existing release output for %s\n' "$target" >&2
    return 1
  fi
  ...
  temporary_archive="$build_root/$archive_base"
  temporary_checksum="$build_root/$archive_base.sha256"
  ...
  tar -C "$work_dir" -czf "$temporary_archive" .
  write_sha256 "$temporary_archive" > "$temporary_checksum"
  publish_new_file "$temporary_archive" "$archive"
  publish_new_file "$temporary_checksum" "$archive.sha256"
  rm -rf -- "$build_root"
